...
import static at.ac.tuwien.aic.sc.web.EnvironmentUtils.isGoogleAppEngine;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (company != null && fromParam != null && toParam != null) {
			...
			if (!company.trim().isEmpty()) {
				...
				Future<Double> future = facade.analyse(c, from, to);
				try {
					session.setAttribute(CURRENT_REQUEST, isGoogleAppEngine() ? future.get() : future);
			...
			if (session.getAttribute(CURRENT_REQUEST) != null) {
				...
				if (obj instanceof Future) {
					Future<Double> analysis = (Future<Double>) obj;
					if (analysis.isDone()) {
						try {
